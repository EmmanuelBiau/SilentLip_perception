%%Generate N-permutations of hit trials from first and second tone conditions; 
clearvars;clc; 
addpath C:\toolbox\CircHist-master; addpath C:\toolbox\Circular_Statistics_Toolbox\; cd XXX\;
...
%% Plot the original data fromequivalent  first and second tones in single tone condition for illustration;  
    
%Plot the mean angle of each participant;
close all;
figure;
subplot(1, 2, 1);
circ_plot2(mean_ori_hit_1Tone_T1,'pretty', [],[],false,true,'linewidth',2,'color','r'); 
title('2Tones - First','Units', 'normalized', 'Position', [0.5, 1.2, 0]);
subplot(1, 2, 2);
circ_plot2(mean_ori_hit_1Tone_T2,'pretty', [],[],false,true,'linewidth',2,'color','r'); 
title('2Tones - Second','Units', 'normalized', 'Position', [0.5, 1.2, 0]);
suptitle('Hits First Tone vs. Second Tone');
text(-4,-1.6,'Comparison zvalues from Rayleigh tests (10000 permutations):')
text(-4,-1.9,['p-value  = ' num2str(P_value_perm)])
hold off
