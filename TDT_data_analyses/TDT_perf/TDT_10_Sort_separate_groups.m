...
%plot phase distributions from the 2 groups;
close all; figure;
ksdensity(mean_hit_T2_cluster1(:,1),-pi:pi/100:pi,'Kernel','normal');
hold on
ksdensity(mean_hit_T2_cluster2(:,1),-pi:pi/100:pi,'Kernel','normal');
xlim([-pi-pi/2 pi+pi/2])
xticks(-pi:pi/2:pi);
hold on
scatter(mean_hit_T2_cluster1(:,1),0.2*ones(length(mean_hit_T2_cluster1(:,1)),1));
hold on
scatter(mean_hit_T2_cluster2(:,1),0.2*ones(length(mean_hit_T2_cluster2(:,1)),1));

%% Plot Dprimes/RTs Clustered data IN TWO TONES CONDITION;
clear;clc;
addpath C:\ANALYSES_DATA\; cd C:\ANALYSES_DATA\scripts_Github
