...
clear;clc; addpath C:\ANALYSES_DATA\scripts_Github;
...
for i = subjects
    ...
    s = s +1;
    hit_sum_oneTone(s,1) = mean(ONE_TONE_ANALYSES.Hit(ONE_TONE_ANALYSES.PARTICIPANT == i & ONE_TONE_ANALYSES.OneTone_position == 1));
    hit_sum_oneTone(s,2) = mean(ONE_TONE_ANALYSES.Hit(ONE_TONE_ANALYSES.PARTICIPANT == i & ONE_TONE_ANALYSES.OneTone_position == 2));
...
%compute the dprimes;
hit_sum_oneTone(hit_sum_oneTone(:,1)==1,1) = 0.995;
hit_sum_oneTone(hit_sum_oneTone(:,2)==1,2) = 0.995;
hit_sum_oneTone(hit_sum_oneTone(:,3)==0,3) = 0.005;
hit_sum_oneTone(hit_sum_oneTone(:,4)==0,4) = 0.005;
...
for i = subjects
    ...
    s = s +1;
    hit_sum_twoTones(s,1) = mean(TWO_TONES_ANALYSES.Hit(TWO_TONES_ANALYSES.PARTICIPANT == i & TWO_TONES_ANALYSES.Tone_position == 1));
    hit_sum_twoTones(s,2) = mean(TWO_TONES_ANALYSES.Hit(TWO_TONES_ANALYSES.PARTICIPANT == i & TWO_TONES_ANALYSES.Tone_position == 2));       
...
%compute the dprimes (with Macmillan & Kaplan(1985) correction); 
hit_sum_twoTones(hit_sum_twoTones(:,1)==1,1) = 0.995;
hit_sum_twoTones(hit_sum_twoTones(:,2)==1,2) = 0.995;
hit_sum_twoTones(hit_sum_twoTones(:,3)==0,3) = 0.005;
hit_sum_twoTones(hit_sum_twoTones(:,4)==0,4) = 0.005;
...
ax.YAxis.Limits = [1.5 5.5];
ax.YTick = [1.5 2.5 3.5 4.5 5.5];
